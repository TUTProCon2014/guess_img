...
class Particle{
	...
	public:
		...
		//Indexの2次元配列に変換する
		std::vector<std::vector<Index2D>> make_indexv(){
			std::vector<std::vector<Index2D>> ret(_problem.div_y(), std::vector<Index2D>(_problem.div_x()));
		...
		void calc_pvalue(){
			...
			int dx[4] = {1, 0, -1, 0};
			int dy[4] = {0, -1, 0, 1};
			std::vector<std::vector<Index2D>> idxs;
			idxs = this->make_indexv(); //index2次元配列を受け取る
				...
				for(int j=0; j < _problem.div_y(); j++){
					for(int k=0; k<4; k++){
						...
						if(sx >= 0 && sx < _problem.div_x() && sy >= 0 && sy < _problem.div_y()){
							...
							double v = std::abs(_f(_problem.get_element(idxs[j][i][0], idxs[j][i][1]),
											       _problem.get_element(idxs[sy][sx][0], idxs[sy][sx][1]),
												  (utils::Direction)k));
							val += v;
						}
					}
...
template <typename BinFunc>
std::vector<std::vector<Index2D>> pso_guess(utils::Problem const & problem, BinFunc f){
